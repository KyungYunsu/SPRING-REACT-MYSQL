...
export default function Header() {
  ...
  const SearchButton = () => {
    ...
    const onSearchBoradChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
      const value = event.target.value;
      setWord(value);
    }
    
    //        event handler: 검색어 키 클릭 이벤트 처리 함수        //
    ...
    if(!status)    
      ...
      <div className='header-search-input-box'>
        <input type="text" className='header-search-input' placeholder='검색어를 입력해주세요.' value={word} onChange={onSearchBoradChangeHandler} onKeyDown={onSearchWordKeyDownHandler} />
  ...
  //          effect: login user 변경 될때 마다 실행될 함수          //
  useEffect(() => {
